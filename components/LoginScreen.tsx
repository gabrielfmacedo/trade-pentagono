...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin, onGoToRegister }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Mesclar membros do storage com os membros padrão do sistema
    const allMembers = [...SYSTEM_DEFAULT_MEMBERS];
    storedMembers.forEach((sm: any) => {
      if (!allMembers.some(am => am.email.toLowerCase() === sm.email.toLowerCase())) {
        allMembers.push(sm);
      }
    });
    ...
    const userInDb = allMembers.find((u: any) => u.email.toLowerCase() === normalizedEmail);
    ...
    const performLogin = () => {
       ...
       if (activeSessions[normalizedEmail] && activeSessions[normalizedEmail] !== sessionId) {
         // Marcar tentativa de multi-login se necessário
             ...
             <div className="relative w-48 h-48 flex items-center justify-center">
                ...
                <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full drop-shadow-[0_0_30px_rgba(16,185,129,0.4)]">
                   ...
                   <g transform="translate(50, 50)">
                     <circle cx="0" cy="0" r="18" fill="none" stroke="url(#goldHighlight)" strokeWidth="2.5" strokeDasharray="5 3" className="opacity-90" />
                     <circle cx="0" cy="0" r="12" fill="url(#goldHighlight)" className="opacity-80" />
                     <path d="M0 -5 L4.75 -1.54 L2.94 4.04 L-2.94 4.04 L-4.75 -1.54 Z" fill="#050505" className="opacity-90" />
                   </g>
                </svg>
             </div>
          </div>
        ...
        <div className="bg-[#0f0f0f] border border-white/5 rounded-[40px] p-10 shadow-[0_40px_80px_-20px_rgba(0,0,0,1)] relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
            <div className="space-y-2">
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="exemplo@email.com" className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white text-sm font-bold outline-none focus:border-emerald-500/50 transition-all placeholder:text-gray-700 shadow-inner" />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] text-gray-500 font-black uppercase tracking-widest px-1">Senha</label>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white text-sm font-bold outline-none focus:border-emerald-500/50 transition-all placeholder:text-gray-700 shadow-inner" />
            </div>
            <div className="flex items-center gap-3 px-1">
              <label className="flex items-center cursor-pointer group">
                ...
                <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest ml-3 group-hover:text-gray-300">Manter conectado</span>
              </label>
            </div>
            {error && <div className="text-red-500 text-[10px] font-black uppercase tracking-widest text-center">{error}</div>}
            <button type="submit" className="w-full py-5 bg-emerald-600 hover:bg-emerald-500 border border-emerald-400 rounded-2xl text-xs font-black uppercase tracking-[0.2em] text-white transition-all shadow-[0_10px_30px_rgba(16,185,129,0.3)] active:scale-95">Entrar no Laboratório</button>
          </form>
        </div>
      </div>
    </div>
